local function project_root()
  ...
  local root = vim.fs.find(
    ...
    { path = vim.fs.dirname(buf), upward = true }
  )[1]
...
╚═╝  ╚═══╝ ╚═════╝   ╚═══╝  ╚═╝  ╚═╝
      ...
      sections = {
        { section = "header", padding = 0 },
        {
          ...
          cmd = [[
            ...
            ForEach-Object {
              Write-Host ("GPU : " + $_.Name + " (" + [math]::Round($_.AdapterRAM / 1GB, 1) + " GB)")
            }
            ...
            Write-Host ("")
            Write-Host ("HDD : " + "[$bar] $usedGB / $totalGB GB")
          ]],
          height = 8,
          padding = 1,
          -- ttl = 5 * 5,
  ...
  keys = {
    ...
    -- git
    {
      "<leader>gb",
      function()
        Snacks.picker.git_branches({ cwd = project_root() })
      end,
      desc = "Git Branches",
    },
    {
      "<leader>gl",
      function()
        Snacks.picker.git_log({ cwd = project_root() })
      end,
      desc = "Git Log",
    },
    {
      "<leader>gL",
      function()
        Snacks.picker.git_log_line({ cwd = project_root() })
      end,
      desc = "Git Log Line",
    },
    {
    ...
    },
    {
      "<leader>gS",
      function()
        Snacks.picker.git_stash({ cwd = project_root() })
      end,
      desc = "Git Stash",
    },
    {
